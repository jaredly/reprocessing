...
let localizePointf (p: (float, float)) (env: glEnv) => Matrix.(matptmul (matinv env.matrix) p);

let localizePoint ((x, y): (int, int)) (env: glEnv) => {
  let (lx, ly) = Matrix.(matptmul (matinv env.matrix) (float_of_int x, float_of_int y));

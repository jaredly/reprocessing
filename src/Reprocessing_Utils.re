...
let remap ::value ::low1 ::high1 ::low2 ::high2 =>
  int_of_float (
    remapf
      value::(foi value) low1::(foi low1) high1::(foi high1) low2::(foi low2) high2::(foi high2)
  );
...
let dist p1::(x1, y1) p2::(x2, y2) => distf p1::(foi x1, foi y1) p2::(foi x2, foi y2);
...
let intersectRectCircle rectPos::(rx, ry) ::rectW ::rectH circlePos::(cx, cy) ::circleRad => {
  let halfW = rectW /. 2.;
  let halfH = rectH /. 2.;
...
let intersectRectRect rect1Pos::(rx1, ry1) ::rect1W ::rect1H rect2Pos::(rx2, ry2) ::rect2W ::rect2H =>
  not (rx2 > rx1 +. rect1W || rx2 +. rect2W < rx1 || ry2 > ry1 +. rect1H || ry2 +. rect2H < ry1);
